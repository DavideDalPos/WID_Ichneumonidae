...
Displays research-grade iNaturalist observation photos for a given taxon, with a responsive grid layout, pagination, and enhanced UX.

This panel resolves taxon matching issues, supports all taxonomic ranks, and ensures only verified, high-quality observations are shown.  
...
- Parses `expanded_name` from TaxonWorks into `{ genus, subgenus, epithet }`.
- Uses iNaturalist `/v1/taxa` endpoint to fetch the **exact `taxon_id`**, avoiding fuzzy matches.
...
- Vertical images are cropped horizontally (`object-cover`) for a uniform appearance.
- Rounded cards with subtle shadows (`rounded`, `shadow-sm`).
...
- Images open safely in a **new tab** (`target="_blank" rel="noopener noreferrer"`).
...
- Automatically updates when the page changes.
...
| Loading spinner | None or basic | `<ClientOnly>` with `<VSpinner>` for better feedback |
| Image layout | Basic grid | Responsive, consistent 3:2 aspect, medium resolution, rounded, hover effect |
| Vertical images | Displayed as-is | Cropped horizontally (`object-cover`) for uniform layout |
